        ...
        for dirpath, _, filenames in os.walk(root_folder):
            for fname in filenames:
                ...
                if full_path in keep_set:
                    continue
                if any(fnmatch.fnmatch(fname, pat) for pat in keep_patterns):
                    continue
                if any(fnmatch.fnmatch(fname, pat) for pat in exclude_patterns):
                    continue
                exclude_files.add(fname)

        exclude_str = ",".join(sorted(exclude_files) + exclude_patterns)
        return exclude_str
            ...
            try:
                ...
                if meta['keep_folder']:
                    ...
                    if meta.get('keep_nfo', False) and "BASE" not in output_filename:
                        ...
                        # Explicitly add subtitle formats to torf include list when keep_folder is used
                        subs = ["*.srt", "*.sub", "*.idx", "*.vtt", "*.ass", "*.ssa", "*.txt"]
                        include.extend([f"{folder_name}/{ext}" for ext in subs])
                        exclude = ["*", "*/**"]
                    ...
                    if meta.get('keep_nfo', False) and not meta.get('is_disc', False) and "BASE" not in output_filename:
                        ...
                        # -- THR Izmjena --
                        # Zadržavamo folder čak i ako je samo 1 video unutra, kako bi mogli uključiti .srt titlove!
