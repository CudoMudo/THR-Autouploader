...
async def upload_image_task(args: Sequence[Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if img_host == "slikethr":
            try:
                payload = {
                    'key': config['DEFAULT']['slikethr_api_key'].strip(),
                    'format': 'json'
                }
            except KeyError:
                return {'status': 'failed', 'reason': 'Missing slikethr_api_key in config'}
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if "image" in res_json and "url" in res_json["image"]:
                            ...
                            raw_url = res_json['image']['url']
                            web_url = res_json['image']['url_viewer'] if 'url_viewer' in res_json['image'] else img_url
                        else:
                            return {'status': 'failed', 'reason': f"Slike.THR response invalid: {res_json}"}
                    else:
                        return {'status': 'failed', 'reason': f"Slike.THR HTTP {response.status_code}: {response.text}"}
            except Exception as e:
                return {'status': 'failed', 'reason': f"Error during Slike.THR upload: {str(e)}"}
