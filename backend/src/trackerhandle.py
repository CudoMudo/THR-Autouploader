...
async def process_trackers(
    ...
    # Automatsko spremanje gotovih torrenta u Torrents folder za korisnika
    import shutil
    import os
    try:
        base_dir = meta.get('base_dir', '')
        uuid_val = meta.get('uuid', '')
        if base_dir and uuid_val:
            seeding_dir = os.path.join(base_dir, "Torrents")
            os.makedirs(seeding_dir, exist_ok=True)
            for tracker in enabled_trackers:
                t_path = os.path.join(base_dir, "tmp", uuid_val, f"[{tracker}].torrent")
                if os.path.exists(t_path):
                    shutil.copy(t_path, seeding_dir)
    except Exception as e:
        if meta.get('debug'):
            console.print(f"[red]Error copying torrent to Torrents folder: {e}[/red]")
