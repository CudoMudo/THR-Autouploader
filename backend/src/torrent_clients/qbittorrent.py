...
from qbittorrentapi.auth import APINames
from qbittorrentapi.exceptions import LoginFailed
from qbittorrentapi.request import Response

# Monkey-patch qbittorrentapi for qBittorrent 5.0.0+ which returns 204 No Content with empty text instead of 'Ok.'
original_auth_log_in = qbittorrentapi.auth.AuthAPIMixIn.auth_log_in

def patched_auth_log_in(self, **kwargs):
    ...
    if auth_response.text not in ("Ok.", ""):
        raise LoginFailed()

qbittorrentapi.auth.AuthAPIMixIn.auth_log_in = patched_auth_log_in
