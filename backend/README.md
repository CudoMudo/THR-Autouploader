...
## THRuploader - Plan Testiranja i Rubni Slučajevi

Ovaj odjeljak služi za praćenje svih rubnih slučajeva i očekivanog ponašanja aplikacije. Ovdje korisnici i developeri mogu potvrditi kako aplikacija reagira u različitim situacijama.

### 1. Osnovni Upload (Bez "Zadrži mapu")
- **Scenarij**: Odabir foldera u kojem se nalazi jedan video file (npr. `.mkv`) i jedan titl (npr. `.srt`).
- **Očekivano ponašanje**: Skripta prepoznaje najveći video file, ignorira titlove/nfo/slike i kreira **single-file torrent** na trackeru (samo `.mkv`).

### 2. Zadrži mapu (Samo video i titlovi)
- **Scenarij**: Odabir foldera s videom i `.srt` titlom + **označena kvačica "Zadrži mapu"**.
- **Očekivano ponašanje**: 
  - U logu se obavezno ispisuje `--keep-folder was specified`.
  - Skripta parsira lokalne fajlove i kreira **folder torrent**.
  - Torrent sadrži folder unutar kojeg su `.mkv` i `.srt`.

### 3. Zadrži mapu (Ignoriranje nepotrebnih fajlova)
- **Scenarij**: Folder sadrži `.mkv`, `.srt`, `.nfo`, `.jpg` i `sample.mkv` + **označena kvačica "Zadrži mapu"**.
- **Očekivano ponašanje**: 
  - Aplikacija kreira folder torrent.
  - Torrent **uključuje**: `.mkv`, `.srt`.
  - Torrent **isključuje**: `.nfo`, `.jpg`, `.png`, `.xml`, i fajlove u čijem je imenu riječ `sample`.

### 4. Prisilno ponovno kreiranje torrenta (Rehash / `-rh`)
- **Scenarij**: U `tmp` mapi već postoji `BASE.torrent` od ranijeg pokušaja, a korisnik ponovno pokreće upload s kvačicom "Zadrži mapu".
- **Očekivano ponašanje**: 
  - Skripta šalje `-rh` (`--rehash`) zastavicu.
  - Aplikacija ignorira stari `BASE.torrent` u `tmp` mapi.
  - Prisilno se pokreće rehash i stvara novi točan torrent.

### 5. Preskakanje qBittorrent pretrage (`-sat`)
- **Scenarij**: Korisnik ima podešen `qbit_url` u `config.py` (npr. na remote seedbox), na seedboxu već postoji taj torrent, ali korisnik želi stvoriti novi (drugačiji) torrent koristeći "Zadrži mapu".
- **Očekivano ponašanje**: 
  - Skripta šalje `-sat` (`--skip-auto-torrent`) zastavicu.
  - Aplikacija **ne pretražuje** remote qBittorrent za postojeći `.torrent` fajl.
  - Aplikacija stvara potpuno novi torrent umjesto da preuzme stari sa seedboxa.

### 6. Čisti start (Tabula Rasa config)
- **Scenarij**: Korisnik je obrisao stari `config.py` i pokreće aplikaciju.
- **Očekivano ponašanje**: 
  - Aplikacija sama kreira prazni defaultni `config.py`.
  - Nema spajanja na nepotrebne remote klijente.
